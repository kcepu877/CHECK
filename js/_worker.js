  ...
  <a href="${telegramku}" target="_self" rel="noopener noreferrer" style="font-family: 'Rajdhani', sans-serif;">
    ...
    <img src="https://bmkg.xyz/img/bot.png" alt="menu" width="50" style="margin-top: 5px;">
                ...
                <div class="form-group">
                    ...
                    <option value="bugwildcard.bmkg.xyz">NO BUG</option>
       ...
       const CHECK_API = `${BASE_URL}/check?ip=`; 
        ...
        if (configType === 'tls') {
            ...
            <span class="flag-circle flag-icon flag-icon-${config.countryCode.toLowerCase()}"></span>
                    ...
                    <td class="button-cell">
                        <button class="px-3 py-1 bg-gradient-to-r from-[#4dbcc7] to-[#4dbcc7] text-black font-semibold border-0 rounded-md transform transition hover:scale-105" onclick="copy('${`trojan://${encodeURIComponent(config.path)}`@${wildcard}:443?encryption=none&security=tls&sni=${modifiedHostName}&fp=randomized&type=ws&host=${modifiedHostName}&path=${encodeURIComponent(config.path)}#(${config.countryCode})%20${config.isp.replace(/\s/g,'%20')}${getFlagEmoji(config.countryCode)}`}')">
                            TROJAN
                        </button>
                    </td>
                    <td class="button-cell">
                        <button class="px-3 py-1 bg-gradient-to-r from-[#ff6e6e] to-[#ff6e6e] text-black font-semibold border-0 rounded-md transform transition hover:scale-105" onclick="copy('${`ss://${btoa(`none:${encodeURIComponent(config.path)}`)}%3D@${wildcard}:443?encryption=none&type=ws&host=${modifiedHostName}&path=${encodeURIComponent(config.path)}&security=tls&sni=${modifiedHostName}#(${config.countryCode})%20${config.isp.replace(/\s/g,'%20')}${getFlagEmoji(config.countryCode)}`}')">
                            Shadowsocks
                        </button>
                    </td>
                </tr>
...
<td class="country-cell"><div class="flag-container">
            ...
            <span class="flag-circle flag-icon flag-icon-${config.countryCode.toLowerCase()}"></span>
                    ...
                    <td class="button-cell">
                        <button class="px-3 py-1 bg-gradient-to-r from-[#4dbcc7] to-[#4dbcc7] text-black font-semibold border-0 rounded-md transform transition hover:scale-105" onclick="copy('${`trojan://${encodeURIComponent(config.path)}`@${wildcard}:80?path=${encodeURIComponent(config.path)}&security=none&encryption=none&host=${modifiedHostName}&fp=randomized&type=ws&sni=${modifiedHostName}#(${config.countryCode})%20${config.isp.replace(/\s/g,'%20')}${getFlagEmoji(config.countryCode)}`}')">
                            TROJAN
                        </button>
                    </td>
                    <td class="button-cell">
                        <button class="px-3 py-1 bg-gradient-to-r from-[#ff6e6e] to-[#ff6e6e] text-black font-semibold border-0 rounded-md transform transition hover:scale-105" onclick="copy('${`ss://${btoa(`none:${encodeURIComponent(config.path)}`)}%3D@${wildcard}:80?encryption=none&type=ws&host=${modifiedHostName}&path=${encodeURIComponent(config.path)}&security=none&sni=${modifiedHostName}#(${config.countryCode})%20${config.isp.replace(/\s/g,'%20')}${getFlagEmoji(config.countryCode)}`}')">
                            Shadowsocks
                        </button>
                    </td>
                </tr>
...
" alt="menu" width="50"></a>
  <select id="wildcard" name="wildcard" onchange="onWildcardChange(event)" style="width: 60px; height: 45px;">
    <option value="bugwildcard.bmkg.xyz" ${!selectedWildcard ? 'selected' : ''}>No Wildcard</option>
...
console.log(`Path: ${pathcfnegara}, Proxy Host: ${proxyHost}, Proxy Port: ${proxyPort}`);
    ...
    if (type === 'trojan') {
      bmkg+= `${ispName},`
      conf += `
${ispName} = trojan, ${bug}, 443, password = ${pathcfnegara} udp-relay = true, skip-cert-verify = true, sni = ${wildcrd}, ws = true, ws-path = ${pathcfnegara}, ws-headers = Host:"${wildcrd}"\n`;
    }
  }
  return `#### BY : FREE CF PROXY #### 
...
console.log(`Path: ${pathcfnegara}, Proxy Host: ${proxyHost}, Proxy Port: ${proxyPort}`);
    ...
    if (type === 'vless') {
      ...
      if (tls) {
        conf += `trojan://${pathcfnegara}@${bug}:443?encryption=none&security=tls&sni=${wildcrd}&fp=randomized&type=ws&host=${wildcrd}&path=${pathcfnegara}#${information}\n`;
      } else {
        conf += `trojan://${pathcfnegara}@${bug}:80?path=${pathcfnegara}&security=none&encryption=none&host=${wildcrd}&fp=randomized&type=ws&sni=${wildcrd}#${information}\n`;
      }
    } else if (type === 'ss') {
      if (tls) {
        conf += `ss://${btoa(`none:${pathcfnegara}`)}%3D@${bug}:443?encryption=none&type=ws&host=${wildcrd}&path=${pathcfnegara}&security=tls&sni=${wildcrd}#${information}\n`;
      } else {
        conf += `ss://${btoa(`none:${pathcfnegara}`)}%3D@${bug}:80?encryption=none&type=ws&host=${wildcrd}&path=${pathcfnegara}&security=none&sni=${wildcrd}#${information}\n`;
      }
    } else if (type === 'mix') {
      if (tls) {
        conf += `vless://${UUIDS}@${bug}:443?encryption=none&security=tls&sni=${wildcrd}&fp=randomized&type=ws&host=${wildcrd}&path=${pathcfnegara}#${information}\n`;
